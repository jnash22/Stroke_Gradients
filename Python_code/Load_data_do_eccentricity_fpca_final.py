...
Data_path = '/Users/josephnashed/Documents/Github_repository/Stroke_Gradients/'
save_path_fig = Data_path + 'Figs_new/'
os.makedirs(save_path_fig+'/All_rois/', exist_ok=True)
os.makedirs(save_path_fig+'/All_rois_unthr/', exist_ok=True)
...
def make_volume_surface(j,converter,atlas, rad = 0.25):
    ...
    rh = Data_path + 'Atlas/Surface/rh.mid_surface.rsl.gii'
    lh = Data_path + 'Atlas/Surface/lh.mid_surface.rsl.gii'

    wrh = Data_path + 'Atlas/Surface/rh.white_surface.rsl.gii'
    wlh = Data_path + 'Atlas/Surface/lh.white_surface.rsl.gii'
...
# Load Data
Salvaged_areas = Data_path + 'test_data/Salvaged.csv'
...
#Load Sample Data
Directory_data = Data_path + 'test_data/'
...
overall_mean_pre = np.nan_to_num(np.tanh(np.mean(np.asarray([np.arctanh(i) for i in m_pre_new]),0)))
...
fig.savefig(save_path_fig+'/component_Variance.png')
plt.close('all')
# plt.show()
#################### COMPONENT VARIANCE ##################
...
##### Converter ######
Atlas = Data_path + 'Atlas/NMT_full.nii.gz'
Atlas_sub_stroke = Data_path + 'Atlas/NMT_Stroke_Atlas_roi.nii.gz'
...
####### CREATE STROKE MASK #############
Stroke_regions = Data_path + 'test_data/Salvaged.csv'
Stroke_mask = converter.inverse_transform(1-pd.read_csv(Stroke_regions).values.squeeze()) 
stroke_surf = nis.vol_to_surf(Stroke_mask ,whole_brain, interpolation='nearest', kind='ball', radius=0.25, inner_mesh=  white_brain,mask_img = ni.binarize_img(Atlas,0))
####### CREATE STROKE MASK #############
...
fig.savefig(save_path_fig+'PC_1_3_eccentricity_pre'+appr+'.png')
# plt.show()
plt.close('all')
...
ax1.set_yticks([-sx_axis, 0, sx_axis])
ax1.set_zticks([-sx_axis, 0, sx_axis])
fig.savefig(save_path_fig+'PC_1_3_eccentricity_post'+appr+'.png')
# plt.show()
plt.close('all')
...
p.add_layer(stroke_surf, cmap='binary',color_range=(0,1)) #stroke_surf_percent
...
# ###################################################### Behavioural Analysis #############################################
# ###################################################### Behavioural Analysis #############################################
Behaviour_path = Data_path + 'test_data/FINAL_NHPSS.csv'
...
fd = skfda.FDataGrid(
    data_matrix= behav.values[:,1::], #behav['NHPSS-Final'].values,
    grid_points=behav.columns.values[1::].reshape(1,-1).astype(int),#behav['NHPSS-Final'].columns.values,
)
...
#### median split
split_val_fpca = np.median(scipy.stats.zscore(scores))
# group positions
...
good_scores = scipy.stats.zscore(scores)[good_group]
poor_scores = scipy.stats.zscore(scores)[poor_group]
...
# locations_in_matrix = new_ind ## index to use for whole brain T-tests
    ...
    print('No ROIs pass FDR in this case')
...
for i in rois_salvaged:
    ...
    for j in rois_salvaged:
        ...
        # good_diff_cr = np.nan_to_num(np.tanh(good_post_corr-good_pre_corr))
        # poor_diff_cr = np.nan_to_num(np.tanh(poor_post_corr-poor_pre_corr))
...
#### typically would FDR threshold the p values as such but nothing will be significant with test data:
#sig_total = [np.where(multitest.multipletests(tot_newp, method='fdr_bh')[0]==True,i,0) for num,i in enumerate(tot_new)] 
#### for illustration purposes only take the non-fdr corrected
sig_total = [np.where(tot_newp[num]<0.05,i,0) for num,i in enumerate(tot_new)]

atlas_GM = ni.load_img(Atlas)
volumes_surface_T_unthr = [make_volume_surface(np.asarray(i),stroke_converter,atlas_GM) for i in total] ###### unthresholded
volumes_surface_T = [make_volume_surface(np.asarray(i),stroke_converter,atlas_GM) for i in sig_total] ###### thresholded
...
########### PLOT the connectivity of each region unthresholded #############
########### PLOT the connectivity of each region unthresholded #############
########### PLOT the connectivity of each region unthresholded #############
########### PLOT the connectivity of each region unthresholded #############
########### PLOT the connectivity of each region unthresholded #############
...
for num,i in enumerate(volumes_surface_T_roi):
    from surfplot import Plot
    max_val = np.max(np.nan_to_num(volumes_surface_T_unthr[num][1]))
    min_val = np.min(np.nan_to_num(volumes_surface_T_unthr[num][1]))

    # print(max_val, min_val,num)
    
    p = Plot(surf_rh=rh,surf_lh=lh,layout='grid',zoom=1.25,size=(800,600))
    if min_val != 0:
        p.add_layer(volumes_surface_T_unthr[num][1], cmap='bwr',color_range=(min_val,-min_val), zero_transparent=True) #stroke_surf_percent
    else:
        p.add_layer(volumes_surface_T_unthr[num][1], cmap='bwr',color_range=(-max_val,max_val), zero_transparent=True) #stroke_surf_percent

    p.add_layer(volumes_surface_T_roi[num][1]*10, cmap='viridis',color_range=(0,1)) #stroke_surf_percent
    p.add_layer(stroke_surf, cmap='binary',color_range=(0,1)) #stroke_surf_percent
    fig = p.build(cbar_kws={'location': 'right', 'label_direction': 90})
    fig.savefig(save_path_fig+'/All_rois_unthr/Sym_new_T_value_ROI_'+str(num)+'.png')
    plt.close('all')
